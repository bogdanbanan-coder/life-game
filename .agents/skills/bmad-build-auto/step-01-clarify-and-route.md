...
1. Load context.
   ...
   3. **Compile epic context if needed.** If no valid cached epic context was loaded, produce `{{config.modules.bmm.implementation_artifacts}}/epic-<N>-context.md` by spawning a subagent synchronously with `[[bmad-snapshot:compile-epic-context.md]]` as its prompt. Pass it the epic number, epics file path, `{{config.modules.bmm.planning_artifacts}}`, and output path `{{config.modules.bmm.implementation_artifacts}}/epic-<N>-context.md`.
