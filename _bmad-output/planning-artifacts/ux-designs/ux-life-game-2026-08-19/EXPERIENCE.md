...
Keyboard input is supported for text and numeric entry only; full keyboard navigation, shortcuts, screen-reader semantics, and alternative input are outside this private mouse-first release. A trackpad is supported through ordinary pointer emulation. Controller, touch, motion, and platform button-glyph adaptation are out of scope with the confirmed desktop/mouse foundation.
...
| Error dialog | Recoverable operation failure, isolated damaged record, or fatal startup database failure | Modal explanation of a specific error. A recoverable operation offers Retry and Cancel; an isolated damaged record offers Acknowledge and returns to its owning surface; a fatal startup error offers Acknowledge and exits. | Owning surface after Cancel/Acknowledge, Retry repeats the operation, or application exit for a fatal startup error. |
...
The Start Screen is both the launch session browser and the Settings gateway. It is not a separate title presentation. A session card preview is only a picture; selecting the card opens the interactive Field Screen. Pressing `Exit` in Field Screen automatically saves the session state and last-camera-view preview, then returns to the Start Screen. Closing the application window while a session is open follows the same automatic-save path as `Exit`. A save or preview failure opens an Error dialog with `Retry` and `Cancel`; `Cancel` leaves the current session open and paused. There is no extra quit-confirmation surface.

When no sessions exist, the Start Screen shows an empty `session-card-list` with an explanation and a direct Create action.
...
The New Session name dialog starts blank. Create stays unavailable while the name is invalid. Cancel does not create a session. A successful create uses the validated Settings width and height, opens the new Field Screen immediately, and starts simulation at the configured interval.

The `bank-list` and `session-card-list` use stable case-insensitive ascending name order. Pointer wheel/trackpad scrolling and a draggable horizontal scrollbar are supported when lists overflow. Empty lists retain their primary action. Closing or canceling Bank does not place a figure; it returns to Live and resumes the simulation with a fresh interval.

## Voice and Tone

Microcopy is brief, literal, and mathematical-tool-like. The following canonical messages are used where the product needs a stable outcome or recovery instruction. Brand posture lives in `DESIGN.md`.
...
| `Figure data could not be read.` | Hide a damaged record or treat it as empty. |
| `Session could not be saved. Retry or Cancel.` | Discard the in-memory session silently. |
| `Settings could not be saved. Retry or Cancel.` | Replace the previous valid Settings silently. |
...
The shipping interface language is English. User-entered names may use Unicode as defined in Naming & Validation.

## Component Patterns

Behavioral rules only; visual specifications live under the identical component identifiers in `DESIGN.md` Components.

The following behaviors are the implementation contract for activation, focus, dismissal, empty states, and error retention.

| Component | Behavioral specification |
|---|---|
| `button` | Hovering does not activate a `button`; pressing and releasing inside the `button` activates it once; disabled actions neither mutate state nor dismiss their surface. Persistent-mode buttons remain selected until another persistent mode is chosen. |
| `panel` | Groups related controls without becoming a navigation target. Pointer events captured by a panel never reach Field cells beneath it. |
...
| `session-card-list` | Scrolls horizontally when cards exceed available width. Pointer wheel/trackpad scrolls horizontally while hovered; a visible scrollbar also supports dragging. The list is stable case-insensitive ascending by name, and its empty state keeps Create available. |
...
| `dialog` | raygui in-application modal. Captures all pointer input. Confirmation and error variants require an explicit action. Figure capture is the only dialog where outside click is defined as Cancel; outside click does not dismiss any other dialog. |
...
| `numeric-field` | Accepts dimensions or positive seconds. It retains invalid input for correction and never silently clamps or rounds it. |
| `bank-list` | Lists the shared Bank by unique name while the Field is paused, in stable case-insensitive ascending order. Selecting a valid row stages that figure; dedicated Rename/Delete actions do not stage it. A damaged figure record remains visible as a disabled row when its identity is available; selecting it shows a specific error. The empty state explains that Highlight saves figures here and retains Close. |
...
| Settings panel | **Clean:** current global Field width, Field height, and generation interval in a two-column table. **Dirty:** Save enabled. **Invalid:** field-local validation and Save disabled. **Save:** atomically validates and commits all three values. **Save failure:** retain edits, preserve the previous stored configuration, and show `Settings could not be saved. Retry or Cancel.` with `Retry` and `Cancel`; Cancel closes the panel without changing stored values. **Cancel:** discard uncommitted edits. Width/height changes affect new sessions only; the current global interval is used by sessions created or opened after Save. |
| Field Screen / Toolbar | **Running:** generation updates active; selected persistent mode visible. **Paused:** no generation updates; Resume visible. **Painting:** touched in-bounds cells change to Live/Die. **Moving:** drag changes camera only. **Bank opened:** RunState changes to Paused, accumulated simulation time is cleared, and Bank controls remain interactive. **Modal/capture/Bank:** unrelated controls are inert. **Auto-save failure:** blocking Error dialog offers `Retry` or `Cancel`; Cancel leaves the session open and paused, with in-memory state retained. |
| Figure-capture dialog | **Open:** selection visible, simulation paused. **Invalid/duplicate name:** selection retained, Save blocked. **Saved:** figure added, dialog closes, Live selected, simulation resumes. **Cancel/outside click:** no Bank mutation, dialog closes, Live selected, simulation resumes. |
...
| Staged-figure overlay | **Valid:** exact rectangle visible with solid outline. **Invalid:** exact rectangle visible with dashed outline and `Outside field`. **Resume valid:** commit exact replacement, Live selected, resume. **Resume invalid:** place nothing, show `Outside field. Nothing placed.`, Live selected, resume. **Pointer outside window:** preserve the last in-window preview position until the pointer re-enters. |
| Confirmation dialog | **Confirm:** destructive action and target are named. **Cancel:** no mutation. No nested modal stack. |
| Error dialog | **Recoverable action failure:** specific error is shown with `Retry` and `Cancel`; Retry repeats the originating action and Cancel returns to the calling surface with valid state preserved. **Damaged-record selection:** specific error is shown with `Acknowledge`, which returns to the calling surface without mutation. **Fatal database open/migration failure:** specific startup error is shown and `Acknowledge` exits the application. No nested modal stack. |
...
- Painting interpolates between pointer samples in cell coordinates so a fast drag does not leave unintended gaps. Leaving the Field while held stops cell changes; re-entering while still held continues the same stroke.
- Pointer capture lasts through a Move or Highlight drag until release, including when the pointer briefly leaves the Field. Only in-bounds endpoints contribute to Highlight; releasing without a valid second in-bounds cell cancels without opening capture.
...
- Zoom anchors on the Field cell under the pointer; if the pointer is outside the Field, zoom anchors on the viewport center.
...
- The stored rectangle's top-left cell anchors to the cell under the pointer. The preview snaps one whole cell at a time. If the pointer leaves the window, the last in-window preview position remains visible.
...
The active mode never fades. The hierarchy is: cell state first, active mode/run state second, available commands third, and feedback fourth. Running/Paused/Staging appears as a short text label in the `toolbar`; the Pause/Resume label alone is not treated as sufficient status. The Field includes no generation counter, coordinates, zoom value, field dimensions, or interval readout because the confirmed sources require none.
...
| Keyboard | Used for focused text and numeric entry only. Enter/Escape shortcuts and full focus traversal are outside the release scope. |
| Trackpad | Ordinary operating-system pointer emulation only; no gestures. |
| Controller / touch / motion | Out of scope. No glyph adaptation or remapping screen is required. |

Pointer cursors are: arrow over menus, crosshair over editable Field in Live/Die/Highlight, open hand in Move, closed hand while moving, and rectangle preview during staging. Cursor shape reinforces the active operation without becoming its only cue.
...
- Every pointer-active overlay captures input so clicks cannot mutate hidden Field cells; modal and toolbar input cannot click through to Field cells.
...
- No Reduce Motion setting is needed because this release defines no non-essential motion. If motion is added later, reduced-motion behavior becomes required.
- Full keyboard-only navigation, screen-reader semantics, and alternative input support are explicitly outside this private mouse-first release; this is an accepted accessibility limitation, not an implicit capability.
...
- Global Settings Field width and height accept positive base-10 whole numbers from 1 through 4096, with each defaulting to 50 cells. Values above the maximum are rejected, not clamped. The total cell-count safety limit is 4,194,304; saved session dimensions do not change when these global defaults are edited.
- The generation interval field accepts a positive base-10 number of seconds with `.` as the decimal separator; the default is 0.25. Zero, negative, nonnumeric, NaN, and infinity are rejected.
- The UI is English-only for this personal release, but Unicode names persist round-trip on both platforms.
...
- Maximum field dimensions, session count, Bank size, figure dimensions, and acceptable operation latency are fixed architecture-owned limits. The application accepts at most 512 sessions and 2048 Bank figures. Internal Release-reference budgets are: default 50×50 simulation/input/render completes within 16 ms; legal maximum-field generation completes within 250 ms; and default-session save plus preview generation completes within 250 ms. These are verification budgets, not a user-facing FPS promise. Session previews use 256×256 PNGs; synchronous operations are measured against those limits rather than presented as live-progress work.

## Key Flows

### Flow 1 — Nerd configures and observes a first experiment

Nerd is a non-story task persona interested only in mathematics. The concrete experiment in this illustrative flow is a three-cell blinker; it does not add a product requirement.
...
7. **Illustrative result:** the three-cell line alternates orientation and returns to its starting orientation after two generations, giving Nerd the mathematical observation sought.
...
The concrete construction in this illustrative flow is the observed blinker; exact preservation and reuse are the focus because no individual experiment was supplied.
...
6. **Illustrative result:** every stored live and dead cell replaces the corresponding cell in the target rectangle at once, cells outside the target rectangle remain unchanged, Live returns, and evolution resumes with an exact reusable copy.
...
Exact restoration of a prior mathematical workspace is the focus because no more specific session goal was supplied.
...
5. **Illustrative result:** the exact construction and view are present and evolving again, while figures captured earlier remain available from the shared Bank.
6. Nerd closes the application window; the application exits. If a session is open, the same automatic-save attempt runs before shutdown.

Failure path: If session data is missing or damaged, Life Game opens a specific error `dialog`, does not silently create an empty replacement, and returns Nerd to the Start Screen after acknowledgment. A save failure before `Exit` blocks the transition, so the current in-memory Field is not silently abandoned; `Retry` repeats the same save and `Cancel` leaves the session open and paused.
...
The following decisions close the implementation-readiness questions identified on 2026-08-24:

1. Zoom anchors on the Field cell under the pointer; outside the Field it anchors on the viewport center.
2. Settings Save atomically commits validated width, height, and interval. Invalid input disables Save. Cancel discards edits. A failed Save preserves the previous stored configuration and keeps the panel open with Retry/Cancel.
3. Window resizing is optional and non-blocking. Text and numeric entry use the keyboard; full keyboard navigation, screen-reader semantics, and alternative input are out of scope for this private mouse-first release.
4. Session and Bank lists use stable case-insensitive ascending order, pointer wheel/trackpad scrolling, and draggable scrollbars. Empty states retain their primary action.
5. Paint strokes interpolate pointer samples; Move/Highlight retain pointer capture until release; staged figures anchor their top-left cell under the pointer; invalid previews use a dashed outline and text; pointer exit retains the last in-window preview.
6. Recoverable operation failures use canonical copy and Retry/Cancel. Isolated damaged-record selection uses specific copy and Acknowledge without mutation. Fatal startup database failures use a specific message and Acknowledge-to-exit. Damaged records remain preserved and visible as disabled where identity is available.
7. Architecture limits are 512 sessions, 2048 Bank figures, and a 4,194,304-cell field safety limit. Internal Release-reference budgets are 16 ms for default-frame work, 250 ms for maximum-field generation, and 250 ms for default-session save plus preview. Visual verification covers contrast, focus, solid/dashed states, text alternatives, and modal click-through.
